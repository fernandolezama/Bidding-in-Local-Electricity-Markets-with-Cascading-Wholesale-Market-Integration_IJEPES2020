...
for Select_Algorithm=1:No_Algorithm
    ...
    lowerB=lowerB*0;upperB=upperB*0;
    otherParameters.WCCI_2020_funct='fitnessFun_WCCI2020';
    otherParameters.lowerB=lowerB;otherParameters.upperB=upperB;
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Call the MH for optimizationclear
    ResDB=struc([]);
    %% Making pictures for the IJEPS2020
...
%% find the best result
    ...
    for iRuns=1:noRuns %Number of trails
        ...
        M_profitCons(iRuns)=sum(profits_a(1:3,1));
        M_profitProsu(iRuns)=sum(profits_a(4:6,1));
        M_profitProd(iRuns)=sum(profits_a(7:9,1));
    ...
    Table_CPrices(Select_Algorithm,:)=LM_CP; %This is the CP of the best solution found with each algorithm
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
BL_sol=size(sol_test);BL_sol=zeros(BL_sol(1),BL_sol(2));[BL, BL_profits,BL_Penalties, BL_TE] = feval('fitnessFun_WCCI2020',BL_sol,caseStudyData,otherParameters);
T_BL_profits=sum(BL_profits);
BL_individual_prof=sum(BL_TE.profits,2);

Table_ave(Select_Algorithm+1,:)=[sum(BL_individual_prof) sum(BL_individual_prof(1:3)) sum(BL_individual_prof(4:6)) sum(BL_individual_prof(7:9)) 0 0 0];
...
figure(ind_plot)
%Joao Suggestion
Balance=(-1.*Matrix_energyBuy(:,1:9,Winner_alg))+(-1.*Matrix_energySell(:,1:9,Winner_alg));
bar(Balance','stacked')
 xlabel('Agents (1 to 3: Consumers; 4 to 6: Prosumers;  7 to 9: Generators )')
 ylabel('Energy (kWh)')
 legend('LEM','Agg+WS')
...
load('Results_IJEPS_rev1\CP_bestSol_PT','CP_bestSol_PT') %It charges Pricetaker solution so PT should be PT=0
